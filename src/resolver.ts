...
export const resolver = (wretcher: Wretcher) => {
  ...
  const catchersWrapper = <T>(promise: Promise<T>): Promise<void | T> => {
    return promise.catch((err) => {
      ...
      else if (catchers.has(error.status)) return catchers.get(error.status)(error, wretcher);
      else if (catchers.has(error.name)) return catchers.get(error.name)(error, wretcher);
      else throw error;
    });
  };
  ...
  const bodyParser: BodyParser = (funName) => (cb: any) =>
    funName
  ...
  const responseChain: ResponseChain = {
     ...
     */
    perfs: (cb) => {
      void fetchRequest.then((res) => perfs.observe(res.url, cb));
