...
export class Wretcher {
  ...
  static factory(url = '', options: WretcherOptions = {}) {
  ...
  url(url: string, replace = false) {
    if (replace) return this.selfFactory({ url });
   ...
   */
  query(qp: Record<string, any> | string, replace = false) {
  ...
  private method(method: string, options = {}, body = null) {
    ...
    let baseWretcher = !body
      ? this
      : typeof body === 'object' &&
        (!headers ||
          Object.entries(headers).every(
            ([k, v]) => k.toLowerCase() !== CONTENT_TYPE_HEADER.toLowerCase() || v.startsWith(JSON_MIME)
          ))
      ? this.json(body)
      : this.body(body);
    baseWretcher = baseWretcher.options({ ...options, method });
    const deferredWretcher = baseWretcher._deferredChain.reduce(
      (acc: Wretcher, curr) => curr(acc, acc._url, acc._options),
      baseWretcher
    );
   ...
   */
  json(jsObject: Record<string, any>) {
    const preservedContentType = Object.entries(this._options.headers || {}).find(
      ([k, v]) => k.toLowerCase() === CONTENT_TYPE_HEADER.toLowerCase() && v.startsWith(JSON_MIME)
    )?.[1];
   ...
   */
  formData(formObject: Record<string, any>, recursive: string[] | boolean = false) {
   ...
   */
  formUrl(input: Record<string, any> | string) {
    return this.body(typeof input === 'string' ? input : convertFormUrl(input)).content(
...
const appendQueryParams = (url: string, qp: Record<string, any> | string, replace: boolean) => {
  let queryString;
  ...
  if (typeof qp === 'string') {
    ...
    for (const key in qp) {
      if (qp[key] instanceof Array) {
        for (const val of qp[key]) usp.append(key, val);
      } else {
...
function convertFormData(
  ...
  Object.entries(formObject).forEach(([key, value]) => {
    ...
    if (value instanceof Array) {
      for (const item of value) formData.append(formKey + '[]', item);
...
function convertFormUrl(formObject: Record<string, any>) {
